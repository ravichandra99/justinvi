...
def date_wise(request):
    ...
    if request.user.is_superuser:
        date_price = [(i,sum([round(float(i.total),2) if i.total==None else (i,0) for i in SaleBillDetails.objects.filter(igst = i)])) for i in date_wise]
    else:
        date_price = [(i,sum([round(float(i.total),2) if i.total==None else (i,0) for i in SaleBillDetails.objects.filter(igst = i).filter(billno__in = saleitems)])) for i in date_wise]
    print(date_price)
